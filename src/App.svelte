<script>
  ...
  // 当前页面状态键名
  const PAGE_STATE_KEY = "linkhub-page-state";
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex">
  ...
  <!-- 主内容区 -->
  <div
    class="flex-1 transition-all duration-300 flex flex-col {sidebarOpen
      ? 'lg:ml-64'
      : ''}"
  >
    <!-- 顶部导航栏 -->
    ...
    <main class="flex-1 p-4 lg:p-6 bg-gray-50 dark:bg-gray-900 overflow-auto">
      ...
      {:else if currentCategory}
        ...
        {:else}
          <div class="mb-6">
            <h1
              class="text-xl font-normal text-gray-600 dark:text-gray-400 mb-2 flex items-baseline gap-2"
            >
              <span
                >{currentSubcategory
                  ? currentSubcategory.subcategory
                  : currentCategory.category}</span
              >
          ...
          <Category
            {categoryData}
            currentCategoryName={currentCategory?.category || ""}
