<script>
  ...
  function persistItemOrder(order) {
    ...
    localStorage.setItem(
      CATEGORY_ITEM_ORDER_STORAGE_KEY,
  ...
  function mergeCategoryOrder(order, categories) {
    ...
    const newOnes = categories.filter(
  ...
  function getRenderSites(category) {
    return reorderMode
      ...
      : (orderedSitesByCategory[category] || []).map((site) => ({
          site,
          id: `${category}:${site.url}`,
        }));
  }
...
<div class="space-y-6">
  ...
  >
    <div class="w-full md:w-auto text-center md:text-left">
      <h1 class="text-xl font-normal text-gray-600 dark:text-gray-400">
        我的收藏
      </h1>
    </div>
    <div
      class="w-full md:w-auto flex flex-wrap gap-2 justify-center md:justify-end"
    >
      <div class="flex flex-wrap gap-2 justify-center">
        <button
          ...
          class={`px-4 py-2 rounded-lg transition-colors duration-200 border ${
            reorderMode
              ? "bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400 border-blue-50 dark:border-gray-700"
        ...
        >
          <i class="fas fa-grip-vertical mr-2"></i>{reorderMode
            ? "完成"
            : "排序"}
        </button>
        <button
          on:click={exportFavorites}
          class="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg transition-colors duration-200"
        >
          <i class="fas fa-download mr-2"></i>导出
        </button>
        <label
          class="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg transition-colors duration-200 cursor-pointer"
        >
  ...
  {#if favorites.length === 0}
    <div class="text-center py-12">
      ...
      <p class="text-gray-600 dark:text-gray-400 mb-8">还没有任何收藏</p>
      <div
        class="max-w-xl mx-auto text-sm text-gray-500 dark:text-gray-500 space-y-1.5"
      >
      ...
      {#if groupedFavorites[category]}
        <div class="category-block">
          <div class="flex items-center justify-between mb-4 gap-2">
            <h2 class="text-base font-normal text-gray-600 dark:text-gray-400">
              {category}
            </h2>
            <div
              class="flex items-center gap-1 text-gray-400 category-sort-controls"
            >
              <button
                type="button"
                class="p-2 rounded-lg border border-transparent hover:border-gray-200 dark:hover:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-40 transition-all duration-200"
                on:click={(event) =>
                  handleMoveButtonClick(event, category, "up")}
                disabled={index === 0}
                aria-label="分类上移"
                title="分类上移"
              >
              ...
              <button
                ...
                class="p-2 rounded-lg border border-transparent hover:border-gray-200 dark:hover:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-40 transition-all duration-200"
                on:click={(event) =>
                  handleMoveButtonClick(event, category, "down")}
          ...
          {#if reorderMode}
            ...
            >
              {#each reorderLists[category] || [] as item (item.id)}
                <div
                  class="reorder-card border border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-3 bg-white/80 dark:bg-gray-900/60 backdrop-blur-sm"
                >
                  <div class="flex justify-end mb-2">
                    <span
                      class="favorite-drag-handle inline-flex items-center gap-1 px-2 py-1 text-xs text-gray-500 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded cursor-grab active:cursor-grabbing"
                    >
