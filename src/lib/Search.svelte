        ...
        if (
          ...
          categoryData.data.forEach((subcategory) => {
            if (
              subcategory &&
              subcategory.sites &&
              Array.isArray(subcategory.sites)
            ) {
              subcategory.sites.forEach((site) => {
                // 搜索网站名称和描述
                if (site && site.name) {
                  const nameMatch = site.name
                    .toLowerCase()
                    .includes(queryLower);
                  const descMatch =
                    site.description &&
...
{#if isMobileExpanded}
  ...
  >
    <div class="px-4 py-3 flex items-center gap-3">
      ...
      <div class="flex-1 relative">
        ...
        <input
          ...
          class="w-full h-12 pl-10 pr-4 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-gray-500 focus:border-transparent text-base"
        />
      </div>
      <!-- 关闭按钮 -->
      <button
        ...
        >
          {#each searchResults as result, index}
            <button
              ...
              class="w-full text-left px-4 py-4 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors border-b border-gray-100 dark:border-gray-700 last:border-b-0 {selectedIndex ===
              index
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  {#if result.description}
                    <div
                      class="text-xs text-gray-600 dark:text-gray-400 mb-1.5 line-clamp-2"
                    >
                      {result.description}
                    </div>
                  {/if}
    ...
    {#if showDropdown && value.trim() && searchResults.length === 0}
      <div class="px-4 pb-4">
        <div
          class="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-6 text-center text-gray-500 dark:text-gray-400"
        >
          未找到匹配结果
        </div>
      </div>
...
<div class="search-container relative flex-1 hidden lg:block">
  ...
  <input
    ...
    class="w-full h-10 pl-10 pr-4 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-gray-500 focus:border-transparent"
  />
    ...
    >
      {#each searchResults as result, index}
        <button
          on:click={() => handleResultClick(result)}
          on:mouseenter={() => handleMouseEnter(index)}
          class="w-full text-left px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors {selectedIndex ===
          index
            ? 'bg-gray-100 dark:bg-gray-700'
            : ''}"
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div
                class="font-semibold text-sm text-gray-900 dark:text-white truncate flex items-baseline gap-2"
              >
  ...
  {#if showDropdown && value.trim() && searchResults.length === 0}
    <div
      class="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg z-50 p-4 text-center text-gray-500 dark:text-gray-400"
    >
      未找到匹配结果
    </div>
  {/if}
