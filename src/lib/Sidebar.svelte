<script>
  ...
  function isCurrentSubcategory(category, subcategory) {
    ...
    const subcategoryMatch =
      currentSubcategory.subcategory === subcategory.subcategory;
  ...
  $: activeSubcategoryKey = (() => {
    ...
    const categoryId =
      currentCategory && currentCategory["id"] ? currentCategory["id"] : null;
...
<!-- 侧边栏 -->
<aside
  class="fixed left-0 top-0 h-full w-64 bg-white dark:bg-gray-800 z-50 transform transition-transform duration-300 ease-in-out {isOpen
    ? 'translate-x-0'
    : '-translate-x-full'}"
>
  <div class="h-full flex flex-col">
    <!-- Logo 区域 -->
    <div
      class="h-16 flex items-center justify-center px-6 bg-white dark:bg-gray-800"
    >
      ...
      <ul class="space-y-2">
        ...
        <li>
          <button
            ...
            class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-colors duration-200 {showFavorites
        ...
        {#each categories as category}
          <li>
            ...
            <button
              ...
              class="w-full flex items-center justify-between gap-3 px-4 py-2.5 rounded-xl transition-colors duration-200 {isCurrentCategory(
                category,
              )
                ? 'bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400'
                : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900 dark:hover:text-gray-200'}"
            >
              ...
              <!-- 箭头图标：收起时向右，展开时向下 -->
              <i
                class="fas fa-chevron-right text-xs transition-transform duration-200 ease-in-out {expandedCategories.has(
                  category.id,
                )
                  ? 'rotate-90'
                  : ''}"
                  ...
                  {#each subcategories as subcategory}
                    ...
                    <li>
                      <button
                        ...
                        class="w-full text-left pl-11 py-2 rounded-lg transition-colors duration-200 text-sm relative {isActive
                          ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-gray-700/30'
                      ...
                      >
                        ...
                        {#if isActive}
                          <span
                            class="absolute left-7 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-blue-500 dark:bg-blue-400 pointer-events-none"
                          ></span>
                        {/if}
